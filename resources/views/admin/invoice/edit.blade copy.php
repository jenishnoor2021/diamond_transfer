...
<div class="card">
  <div class="card-body">
    ...
    <form method="POST" id="invoiceForm" action="{{ route('admin.invoice.update', $invoice->id) }}">
      @csrf
  ...
  <button type="button" class="btn btn-info mt-3" id="previewBtn">Preview</button>
...
<script>
      ...
      success: function(res) {
        ...
        let row = `
          <tr>
              <td>
                  ${res.stock_no}
                  <input type="hidden" name="diamond_id[]" value="${res.id}">
              </td>
              <td><input type="number" name="weight[]" value="${res.weight}" class="form-control"></td>
              <td><input type="number" name="rate[]" value="${res.rate}" class="form-control"></td>

              <td class="sgst_col d-none">0</td>
              <td class="cgst_col d-none">0</td>
              <td class="igst_col d-none">0</td>

              <td><input type="text" class="form-control amount" value="${res.price}" readonly></td>
              <td><button type="button" class="btn btn-danger remove">X</button></td>
          </tr>`;

        $('#diamond_table tbody').append(row);
        manageTaxColumns();
        calculateTotal();
      }
    });
  ...
  function isDuplicateDiamond(id) {
    let exists = false;

    $('input[name="diamond_id[]"]').each(function() {
      if ($(this).val() == id) {
        exists = true;
      }
    });
  ...
  $('form').on('submit', function() {
    ...
    $(this).attr('action', "{{ route('admin.invoice.store') }}");
  });
